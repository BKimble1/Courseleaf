...
| GitHub Actions `macos-15` (Xcode 16.x, iOS 18 SDK, iPad simulator) | app compiles; XCTest bundle runs on an iPad simulator (PencilKit/PDFKit/Vision code paths that do not need a real Pencil) | job `app-ios-simulator`; results recorded in `docs/VALIDATION.md` |
...
2. `app-ios-simulator`: prints `xcodebuild -version` and available SDKs and
   simulators, installs XcodeGen, generates the project, builds the app for an
   iPad simulator with code signing disabled, runs `CourseleafTests`, and
