...
|---|---|---|---|---|
| 1 | Finish and verify launch gates G0–G6 (`docs/FEATURE_REGISTER.md`, `docs/VALIDATION.md` A01–A20) | developer | in progress | The core package and every `App/` screen are written and green in CI; see `docs/FEATURE_COVERAGE.md` for what each category does and does not do. Device gates A02, A03, A06, A16, A17, A19 need a physical iPad and an Apple Pencil, which this project has never had |
...
| 12 | Screenshots from the implemented app (12.9" and 11" iPad, portrait and landscape) | developer (Mac + simulator or device) | in progress | `CourseleafUITests` attaches real simulator screenshots (default, highlighter, colour and width, a favourite, the options menu, landscape, portrait, dark, accessibility text size) to the `ios-simulator-results` artifact on every run. Store-sized, store-framed screenshots for each required device class are still to be produced. Only real screens; no mock-ups |
...
| 22 | Upload to TestFlight internal testing | developer (authorized) | done, repeatable | **1.0.0 (2)** from `45489d7` reached `VALID` on 2026-09-13 (build id `2ea85ecb-94c5-4b5e-95a1-68477d57ba44`, [run 34728597036](https://github.com/BKimble1/Courseleaf/actions/runs/34728597036)), and App Store Connect lists it in the group. `.github/workflows/testflight.yml`, manual dispatch. Uploads, waits for processing to reach `VALID`, sets the build's **What to Test** from `docs/release/WHAT_TO_TEST.md`, then adds the build to the **Courseleaf Testing Group** internal group — the app's one internal group, and Apple's exact name for it. (It read "Personal Testing" until a release reached the assignment step and found no such group.) **Submitting for public App Store review is a separate, explicit action in App Store Connect and is not automated here** |
| 23 | Post-submission: tag the commit, archive the `.xcarchive` and dSYMs, update `docs/NEXT_SESSION.md` | developer | not started | |
| 24 | CI green on the release commit: `core-linux` and `app-ios-simulator` (`.github/workflows/courseleaf.yml`) | developer | done for 1.0.0 (2) | Both jobs now guard themselves against a false green: the macOS job through `Scripts/check-test-results.sh`, the Linux job through `swift test`'s xUnit report (it used to pipe into `tee` and report tee's exit code). Record both run URLs in `VALIDATION.md` A20 for the commit that is actually released |
...
The tests it runs are the whole simulator suite, unit **and** UI
(`CourseleafTests` and `CourseleafUITests`). The UI tests launch the app on a
throwaway library in a temporary directory, so they never touch a real one, and
they attach screenshots of what they saw to the result bundle.

### What testers read

After the build reaches VALID and before it is added to the group, the workflow
sets the build's **What to Test** from `docs/release/WHAT_TO_TEST.md`
(`Scripts/asc.py set-notes`, which updates the existing localisation rather than
adding a second). **Edit that file as part of the change it describes**, not
afterwards: it is the only place a build says what is new, what is off by
default and what is known to be unfinished, and a tester reads it before they
decide to install.

### If the upload works but the group assignment does not

Run **TestFlight assignment** (`testflight-assign.yml`) with the build number.
It talks only to App Store Connect, runs on Linux in under a minute and builds
nothing, so it cannot change what shipped. Never re-run the whole release to
retry two API calls: that re-archives and re-signs a binary Apple has already
accepted, and this repository's macOS runners fail often enough that it is a
worse bet than the thing that already worked.
